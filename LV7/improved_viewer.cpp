...
bool ImprovedViewer::key_press_event(int key, int modifiers) {
    ...
    else if(key == GLFW_KEY_T)
    {
       /* pts.emplace_back(0,0,0);
        pts.emplace_back(2,1,0);
        pts.emplace_back(1,2,0);
        pts.emplace_back(-1,3,0);
        pts.emplace_back(-1,2,0);
        ...
        {
            z.push_back(pts[i].x * pts[i].x + pts[i].y * pts[i].y);
            tacke.push_back(pts[i]);
        }

        for(unsigned long i = 0; i < pts.size()-2; ++i)
        {
            for(unsigned long j = i+1; j < pts.size(); ++j)
            {
                for(unsigned long k = i+1; k < pts.size(); ++k) {
                    if (j != k)
                    {
                        float u = (pts[j].y - pts[i].y) * (z[k] - z[i]) - (pts[k].y - pts[i].y) * (z[j] - z[i]);
                        float v = (pts[k].x - pts[i].x) * (z[j] - z[i]) - (pts[j].x - pts[i].x) * (z[k] - z[i]);
                        float w = (pts[j].x - pts[i].x) * (pts[k].y - pts[i].y) -
                                  (pts[j].y - pts[i].y) * (pts[k].x - pts[i].x);
                        int m = 0;
                        bool t = w < 0;
                        while (m < pts.size() && t)
                        {
                            t = (pts[m].x - pts[i].x) * u + (pts[m].y - pts[i].y) * v + (z[m] - z[i]) * w <= 0;
                            ++m;
                        }
                        if (t)
                        {
        ...
        auto linije = new LinesDrawable("triangulacija");
        linije->update_index_buffer(ind);
        linije->update_vertex_buffer(tacke);
        linije->set_default_color(vec3(1,0,0));
        this->add_drawable(linije);
        this->fit_screen();
        camera()->showEntireScene();
        update_rendering();
